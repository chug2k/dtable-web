...
import { siteRoot, gettext, appAvatarURL, isOrgContext } from '../../utils/constants';
...
class Account extends Component {
    ...
    if (this.props.isAdminPanel) {
      ...
      if (isStaff) {
        data = {
          ...
          text: gettext('Enter System Admin')
        };
      } else if (isOrgStaff) {
        data = {
  ...
  render() {
    return (
      <div id="account">
        <span id="my-info" onClick={this.onClickAccount} className="account-toggle no-deco d-none d-md-block" aria-label="View profile and more">
          <span>{this.renderAvatar()}</span>
          <span className="dtable-font dtable-icon-drop-down vam"></span>
        </span>
        <span className="account-toggle dtable-font dtable-icon-more-vertical mobile-icon d-md-none" aria-label="View profile and more" onClick={this.onClickAccount}></span>
