...
class SysAdminShareToUser extends React.Component {
  ...
  componentDidMount() {
    ...
    dtableWebAPI.sysAdminListRepoSharedItems(repoID, 'user').then((res) => {
  ...
  shareToUser = () => {
    ...
    dtableWebAPI.sysAdminAddRepoSharedItem(repoID, 'user' , users, this.state.permission).then(res => {
      let errorMsg = [];
  ...
  deleteShareItem = (useremail) => {
    let repoID = this.props.repoID;
    dtableWebAPI.sysAdminDeleteRepoSharedItem(repoID, 'user', useremail).then(res => {
      this.setState({
  ...
  onChangeUserPermission = (item, permission) => {
    ...
    dtableWebAPI.sysAdminUpdateRepoSharedItemPermission(repoID, 'user', userEmail, permission).then(() => {
