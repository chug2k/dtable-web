...
def work_weixin_oauth_callback(request):
    ...
    work_weixin_oauth_state = request.session.get('work_weixin_oauth_state', None)
    work_weixin_oauth_redirect = request.session.get('work_weixin_oauth_redirect', redirect_to)
    # clear session
    try:
        del request.session['work_weixin_oauth_state']
        del request.session['work_weixin_oauth_redirect']
    except Exception as e:
        logger.warning(e)

    # get api user
    if state != work_weixin_oauth_state or not code:
...
def work_weixin_oauth_connect_callback(request):
    ...
    work_weixin_oauth_connect_state = request.session.get('work_weixin_oauth_connect_state', None)
    work_weixin_oauth_connect_redirect = request.session.get('work_weixin_oauth_connect_redirect', redirect_to)
    # clear session
    try:
        del request.session['work_weixin_oauth_connect_state']
    ...
    code = request.GET.get('code', None)
    state = request.GET.get('state', None)
    if state != work_weixin_oauth_connect_state or not code:
        logger.error('can not get right code or state from work weixin request')
        return render_error(request, _('Error, please contact administrator.'))
